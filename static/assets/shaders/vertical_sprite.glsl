...
void main() {
    v_uv = a_pos;
    vec2 quad_pos = (a_pos - vec2(0.5, 0.0)) * u_size;
    vec3 local_pos = vec3(quad_pos.x, 0.0, quad_pos.y);
    local_pos.xy = rotate(local_pos.xy, u_rot);
    vec3 world_pos = u_pos + local_pos;
